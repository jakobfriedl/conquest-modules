...
cmd_noconsolation = (
    conquest.createCommand(name="no-consolation", description="Execute an unmanaged PE in memory.", example="no-consolation --local C:\\Windows\\System32\\calc.exe\n         no-consolation /mnt/c/tools/precompiled-binaries/Credentials/mimikatz.exe coffee exit",
            ...
            .addFlagBool("--local", "Load the binary from the target Windows machine.")
            .addFlagBool("--inthread", "Run the PE with the main thread (may hang the agent).")
            .addFlagBool("--link-to-peb", "Load the PE into the PEB.")
            .addFlagBool("--dont-unload", "Don't unload the DLL after execution.")
            ...
            .addFlagBool("-k", "Overwrite the PE headers.")
            .addFlagString("--method", "method", "Export method/function name to execute for DLLs (default: DllMain).", False, "DllMain")
            .addFlagBool("-w", "Pass command line arguments in UNICODE format (default: ANSI).")
            .addFlagBool("--no-output", "Do not capture output.")
            .addFlagBool("--alloc-console", "Allocate a console (spawns new process).")
            .addFlagBool("--close-handles", "Close pipe handles after execution.")
            .addFlagString("--free-libs", "FL_DLLS", "Comma-separated list of DLLs to offload from memory.")
            .addFlagBool("--dont-save", "Do not save this binary in memory.")
            .addFlagBool("--list-pes", "List all PEs loaded in memory.")
            .addFlagString("--unload-pe", "PE_NAME", "Unload a specific PE from memory by name.")
            .addFlagBool("--load-all-deps", "Custom load all PE dependencies.")
